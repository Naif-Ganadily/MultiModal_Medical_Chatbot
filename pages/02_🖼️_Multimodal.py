...
def main():
    ...
    missing_models = set(["Med-Palm2:latest", "Med-Flamingo2:latest"]) - set(available_models)

    col_1, col_2 = st.columns(2)
    with col_1.popover("⚙️ Model Management", help="Manage models here"):
        if not available_models:
            st.warning("No allowed models are available", icon="⚠️")
            model_to_download = st.selectbox(
                 "Select a model to download", ["Med-Palm2:latest", "Med-Flamingo2:latest"]
            )
            ...
            if missing_models:
                model_to_download = st.selectbox(
                    ":green[**mailbox_with_mail:**] Select a model to download",
                    list(missing_models)
                )
                if st.button(f":green[**mailbox_with_mail:**] Download {model_to_download}_**"):
                    try:
                        ollama.pull(model_to_download)
                        st.toast(
                            f"""Download model: {
                        ...
                        st.error(
                            f"""Failed to download model: {
                                model_to_download}. Error: {str(e)}""",
                                icon="⛔",)
        selected_models = st.selectbox(":red[**⛔ DELETE MODEL**]", available_models)
        if st.button(f"Delete **_{selected_models}_**", type="primary"):
            try:
                ...
                st.error(f"Failed to delete model: {selected_model}. Error: {str(e)}", icon="⛔")  
        ...
        with col1:
            ...
            if uploaded_file is not None:
                ...
                if user_input := st.chat_input(
                    ...
                    with container2.chat_message("assistant", avatar="🖼️"):
                        with st.spinner(":blue[**🤖 Analyzing...]"):
                                ...
                                if line.strip():
                                    try:
                                        ...
                                        if "response" in response_data:
                                            medai_response += response_data["response"]
                                    except json.JSONDecodeError:
                                        pass
                            if medai_response:
                                st.markdown(medai_response)
                            else:
                                st.error(
                                    f"""No response from {selected_model}.""",
                                    icon="⛔",
                                )
                        else:
                            st.error(
                                f"""Failed to generate response from {selected_model}.""",
                                icon="⛔",)

                    st.session_state.chats.append(
                        {"role": "assistant", "content": medai_response}
                    )
